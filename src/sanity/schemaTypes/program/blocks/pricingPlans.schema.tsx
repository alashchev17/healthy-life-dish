import { defineField, defineType, PatchEvent, set, useFormValue } from "sanity";
...
export const pricingPlansSchema = defineType({
  ...
  fields: [
        ...
        input: function PricingPlansPreviewInput(props) {
          ...
          return (
            <BlockPreview
              ...
              altText={
                hasGroupPricing
                  ? "Пример блока тарифных планов с групповыми ценами"
                  : "Пример блока индивидуальных тарифных планов"
              }
            />
          );
        },
      ...
      of: [
        {
          ...
          fields: [
            defineField({
              ...
              description:
                "Автоматически сгенерированный идентификатор для использования в системе оплаты",
            ...
            defineField({
              ...
              type: "string",
              description:
                "ID продукта в Stripe (заполняется при интеграции с платежной системой)",
              hidden: ({ currentUser }) => {
                return !currentUser?.roles.find(
                  (role) => role.name === "administrator",
                );
            ...
            defineField({
              ...
              type: "object",
              description:
                "Укажите цену за 1 человека за 1 месяц для каждой валюты",
              validation: (Rule) => Rule.required(),
              fields: [
                defineField({
                  name: "EUR",
          ...
          preview: {
            ...
            prepare({ title, pricing, isPopular, id }) {
              const firstPrice =
                ...
                pricing?.GBP?.price ||
                pricing?.UAH?.price;
              const currency = pricing?.EUR?.price
                ? "EUR"
                : pricing?.USD?.price
                  ? "USD"
                  : pricing?.GBP?.price
    ...
    defineField({
      ...
      of: [
        {
          ...
          fields: [
            ...
            defineField({
              ...
              type: "object",
              description:
                "Укажите базовую цену и опциональную цену со скидкой для каждой валюты",
              validation: (Rule) => Rule.required(),
              fields: [
                defineField({
                  name: "EUR",
          ...
          preview: {
            ...
            prepare({ people, pricing }) {
              const firstCurrency =
                pricing?.EUR || pricing?.USD || pricing?.GBP || pricing?.UAH;
              const currencyCode = pricing?.EUR
                ? "EUR"
                : pricing?.USD
                  ? "USD"
                  : pricing?.GBP
      ...
      description:
        "Настройте тарифы для групп (2+ человек). Для каждой группы укажите цены по валютам.",
    }),
  ],
});
